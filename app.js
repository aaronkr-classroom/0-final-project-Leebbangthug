...
// 데이터베이스 연결 설정
mongoose.connect(
  "mongodb+srv://ut-node:cherry0909!@ut-node.w4u3y6k.mongodb.net/?retryWrites=true&w=majority&appName=ut-nodee", // 데이터베이스 연결 설정 Atlas 경로 (lesson-15)
);
...
app.post('/signup', [
  ...
  body('Description').notEmpty().withMessage('Description cannot be empty').trim(),
  body('Max Students').isInt({ min: 1 }).withMessage('Max Students must be a positive integer'),
